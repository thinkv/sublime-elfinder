...
from os.path import dirname, realpath, join, split
packagePATH, elfinderNAME = split(dirname(realpath(__file__)))
...
class ElementFinderCommand(sublime_plugin.WindowCommand):
	...
	def on_css_selector_entered(self, selector):
		...
		self.output_view.set_syntax_file("Packages/" + elfinderNAME + "/Element Finder Results.tmLanguage")
		self.output_view.settings().set("result_file_regex", "^([^ ].*) \([0-9]+ match(?:es)?\)$")
		self.output_view.settings().set("result_line_regex", "^ +([0-9]+): ")
		
		# Workaround for issue https://github.com/wuub/SublimeREPL/issues/220#issue-17251920
	...
	def print_line(self, output):
		self.output_view.run_command('insert', {'characters': output})
		...
		try:
			self.sp = subprocess.Popen(
				[
					...
					sublime.packages_path() + "/" + elfinderNAME + "/lib/element-finder/bin/element-finder.js",
	...
	def processLine(self, line):
		...
		if "status" in json_line:
			if json_line["status"] == "complete":
				self.complete = True
		if "numberOfFiles" in json_line:
			if json_line["numberOfFiles"] == 0:
				self.complete = True